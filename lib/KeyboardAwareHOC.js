...
function KeyboardAwareHOC(
  ...
  return class
    ...
    isFocusedInputMultiply = (id) => {
      const { multilineInputs } = this.props;
      if (multilineInputs && multilineInputs.length) {
        return multilineInputs
          .filter(input => input.state && input.state.target === id)
          .length > 0;
      }
    }
    ...
    _scrollToFocusedInputWithNodeHandle = (
      ...
      if (!this.isFocusedInputMultiply(nodeID)) {
        return;
      }
      if (extraHeight === undefined) {
